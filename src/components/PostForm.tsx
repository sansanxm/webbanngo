...
export default function PostForm({ initialData, fixedCategory, redirectBaseUrl = "/admin/posts", categoryOptions, onDelete }: PostFormProps) {
    const router = useRouter();
    const [title, setTitle] = useState(initialData?.title || "");
    const [introduction, setIntroduction] = useState(initialData?.introduction || "");
    const [author, setAuthor] = useState(initialData?.author || "");
    const [category, setCategory] = useState(fixedCategory || initialData?.category || "Tin tức chung");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (initialData?.id) {
                // If updating, do NOT update the date to preserve order
                await updateDoc(doc(db, "posts", initialData.id), postData);
            } else {
                // If creating, add the current date
            ...
            setError("Lưu bài viết thất bại: " + (err.message || "Lỗi không xác định"));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
            ...
            <div>
                <label className="block text-sm font-bold text-black mb-1">Tiêu đề</label>
                <input
                    type="text"
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    required
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black font-medium"
                />
            </div>

            <div>
                <label className="block text-sm font-bold text-black mb-1">Đoạn mở đầu (Introduction)</label>
                <textarea
                    value={introduction}
                    onChange={(e) => setIntroduction(e.target.value)}
                    rows={3}
                    placeholder="Tóm tắt nội dung bài viết (3-5 dòng), sẽ được hiển thị in đậm..."
                    onKeyDown={(e) => {
                        if (e.key === 'Tab') {
                            e.preventDefault();
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black font-medium"
                />
                <p className="mt-1 text-xs text-gray-500">Đoạn này sẽ hiển thị <b>in đậm</b> ngay dưới tiêu đề và trước ảnh đại diện.</p>
            </div>

            <div>
                <label className="block text-sm font-bold text-black mb-1">Tác giả</label>
                <input
                    type="text"
                    value={author}
            ...
            {!fixedCategory && (
                <div>
                    <label className="block text-sm font-bold text-black mb-1">Danh mục</label>
                    <select
                        value={category}
                    ...
                    >
                        {categoryOptions ? (
                            ...
                            <>
                                ...
                                <option value="Y tế - Thư viện">Y tế - Thư viện</option>
                                <option value="Hoạt động">Hoạt động khác</option>
                                <option value="Thông báo">Bảng tin (Thông báo)</option>
                            </>
                        )}
                    </select>
                </div>
            ...
            <div>
                <label className="block text-sm font-bold text-black mb-1">Ảnh đại diện</label>
                ...
                <div className="text-xs text-gray-500 font-bold mb-2 uppercase text-center">- HOẶC TẢI LÊN -</div>
            ...
            <div>
                <label className="block text-sm font-bold text-black mb-1">Liên kết đính kèm (Video/Website)</label>
                <input
                    type="url"
                    value={embeddedLink}
            ...
            <div>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-2">
                    <label className="block text-sm font-bold text-black">Nội dung</label>
                    <div className="flex flex-wrap items-center gap-2">
                        <button
                            ...
                            onClick={() => {
                                ...
                                const newCursorPos = start + `<b>${selected}</b>`.length;
                                setContent(newContent);
                                setTimeout(() => {
                                    textarea.focus();
                            ...
                            className="p-1 px-2 border rounded hover:bg-gray-100 text-sm font-bold text-black"
                            title="In đậm"
                        >
                            B
                        </button>
                        <button
                            ...
                            onClick={() => {
                                ...
                                const newCursorPos = start + `<i>${selected}</i>`.length;
                                setContent(newContent);
                                setTimeout(() => {
                                    textarea.focus();
                                    textarea.setSelectionRange(newCursorPos, newCursorPos);
                            ...
                            className="p-1 px-2 border rounded hover:bg-gray-100 text-sm italic text-black"
                            title="In nghiêng"
                        >
                            I
                        </button>
                        <button
                            ...
                            onClick={() => {
                                ...
                                const newCursorPos = start + `<u>${selected}</u>`.length;
                                setContent(newContent);
                                setTimeout(() => {
                                    textarea.focus();
                                    textarea.setSelectionRange(newCursorPos, newCursorPos);
                            ...
                            className="p-1 px-2 border rounded hover:bg-gray-100 text-sm underline text-black"
                            title="Gạch chân"
                        >
                            U
                        </button>
                        <button
                            type="button"
                            onClick={() => {
                                const url = window.prompt("Nhập link ảnh (Google Drive hoặc URL khác):");
                                if (url) {
                                    const directUrl = getDriveDirectLink(url);
                                    setContent(content + `\n[img: ${directUrl}]\n`);
                                }
                            }}
                            className="p-1 px-2 border rounded hover:bg-gray-100 text-sm font-bold text-blue-600"
                            title="Chèn Link Ảnh"
                        >
                            LINK
                        </button>
                        <button
                            ...
                            className="text-sm text-blue-600 hover:text-blue-800 flex items-center gap-1 ml-2 font-semibold"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <textarea
                            ...
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-64 md:h-[500px] text-base leading-relaxed text-black bg-white font-medium"
                            placeholder="Nhập nội dung bài viết..."
                        />
                        <p className="mt-1 text-xs text-gray-500">Hỗ trợ: <b>In đậm</b>, <i>In nghiêng</i>, <u>Gạch chân</u>, [img: url], Phím Tab thụt đầu dòng</p>
                    </div>
                    <div className="border rounded-lg p-4 h-[500px] overflow-y-auto bg-gray-50">
                        <h3 className="text-sm font-bold text-gray-500 mb-2 uppercase border-b pb-1">Xem trước</h3>
